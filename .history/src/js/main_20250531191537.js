...
try {
    ...
    const nameInput = document.getElementById("#contact__input_name");
    const emailInput = document.getElementById("#contact__input_email");
    const checkbox = document.getElementById("#contact__input_checkbox");

    form.addEventListener("submit", (e) => {
    e.preventDefault();

    // ⬇️ Вот чего не хватает — получаем значения полей прямо внутри обработчика
    ...
    if (errors.length > 0) {
        ...
        // Тут можешь отправить форму через fetch или form.submit()
    }
});
